...
\begin{frame}{¿Por qué salida estructurada? Intuición}
	\textbf{El LLM no es el producto final.} Es \emph{un nodo} dentro de un
	grafo (LangGraph) que consume y produce datos tipados:
	\texttt{ConsolidatedReport}, \texttt{Scorecard}, \texttt{RoadmapActions}.
	Texto libre rompe el pipeline.

	\medskip
	\textbf{Por qué imponemos un esquema fijo.}
	\begin{itemize}
		\item \textbf{Reproducibilidad}: misma entrada $\Rightarrow$ misma
		      forma de salida, comparable entre corridas y entre pueblos.
		\item \textbf{Alineación}: cada campo codifica una decisión editorial
		      (qué reportar, con qué granularidad, qué taxonomía usar).
		\item \textbf{Validación automática}: rangos
		      (\texttt{score}\,$\in[1,10]$), tipos, listas no vacías — sin
		      parsers tolerantes ni heurísticas de reparación.
		\item \textbf{Composabilidad}: la rama de auditoría y el chat consumen
		      los mismos objetos sin volver a parsear.
	\end{itemize}

	\smallskip
	{\footnotesize El \emph{cómo} (constrained decoding + Pydantic) se
